...
const { getFollowingUserList } = require("../models/followModel");
...
const getBlogsController = async (req, res) => {
  ...
  try {
    ...
    const followingUserIds = followingUserList.map((obj) => obj._id);

    const blogsDb = await getBlogs({ SKIP, followingUserIds });
